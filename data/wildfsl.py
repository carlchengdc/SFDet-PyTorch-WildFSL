...
# change to wildfsl classes, all instances of voc classes
WILDFSL_CLASSES = ('a', 'b', 'c', 'd', 'e', 'f',
               'g', 'h', 'i', 'j', 'k', 'l', 'm',
               'n', 'ñ', 'o', 'p', 'q',
               'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z')
...
class VOCAnnotationTransform(object):
    ...
    def __init__(self,
        ...
        self.class_to_index = dict(zip(WILDFSL_CLASSES, range(len(WILDFSL_CLASSES))))
...
class WildFSL(Dataset):
	...
	# all filenames of all images
        self.ids = []

        # replace with directory of your test, train, and validation
        if self.mode == "test":
            directory = "C:\\Users\\Carl\\OneDrive - De La Salle University - Manila\\Dataset\\In-the-wild FSL Alphabet Dataset.v6i.voc\\test"
        elif self.mode == "train":
            directory = "C:\\Users\\Carl\\OneDrive - De La Salle University - Manila\\Dataset\\In-the-wild FSL Alphabet Dataset.v6i.voc\\train"
        elif self.mode == "validation":
            directory = "C:\\Users\\Carl\\OneDrive - De La Salle University - Manila\\Dataset\\In-the-wild FSL Alphabet Dataset.v6i.voc\\validation"
        
        for foldername, subfolders, filenames in os.walk(directory):
            for filename in filenames:
                if filename.endswith(".jpg"):
                    truncated_filename = filename[:-4]  # Remove the last 4 characters (".jpg")
                    self.ids.append(truncated_filename)
    ...
    def pull_item(self,
        ...
        # path to the xml file of the selected image
        for foldername, subfolders, filenames in os.walk(directory):
            for xml_file in filenames:
                if xml_file == f"{image_id}.xml":
                    target_path = os.path.join(foldername, xml_file)
        
        # path to image
        for foldername, subfolders, filenames in os.walk(directory):
            for jpg_file in filenames:
                if jpg_file == f"{image_id}.jpg":
                    image_path = os.path.join(foldername, jpg_file)
...
def save_results(all_boxes,
    ...
    # for each class
    for class_i, class_name in enumerate(WILDFSL_CLASSES):
...
def do_python_eval(results_path,
    ...
    aps = []
    for class_name in WILDFSL_CLASSES:
